...
public class JZipProject {
    ...
    public JZipProject(String folder_path, String path, String path_jdk) {
        ...
        // findFiles(folder_path, "*.zip", list); // *.zip
        findFiles(folder_path); // *.zip
    }
    ...
    public void parse(int idx_start, int idx_end, boolean compressed) throws IOException {
        for (int i = idx_start; i < idx_end; i++) {
            ...
            try{
                ...
                if (compressed){
                    jZipFolder.zipParse(jOutput);
                }else {
                    jZipFolder.parse(jOutput);
                }
                System.out.println(file.getAbsolutePath() + " parsed !");
            }catch (Exception e){
                System.out.println(file.getAbsolutePath() + " cannot be parsed !");
    ...
    private void findFiles(String path, String name) {
        String tempName;
        File baseDir = new File(path);
        if (!baseDir.exists() || !baseDir.isDirectory()) {
            System.out.println("failed:" + path + "not a directory!");
